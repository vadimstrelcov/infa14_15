...
void Polygon::solve(double time_start, const int NT, bool parallel) {
	int nodes, rank;
	MPI_Comm_size(MPI_COMM_WORLD, &nodes);
	MPI_Comm_rank(MPI_COMM_WORLD, &rank);

	this->log = this->log && (rank == 0);
	if (this->log) {
		this->file_log=fopen("log.txt", "at+");
	}
	...
	for (int tau=0;tau<NT;tau++) {
		if (parallel){
			get_tmp_answer_parallel(&(this->U),&tmpU,time_start+tau*this->h_t,0);
			data_exchange(&tmpU, 0);
			get_tmp_answer_parallel(&tmpU,&(this->U),time_start+tau*this->h_t,1);
			data_exchange(&(this->U), 1);
		}
		else{
...
void Polygon::get_tmp_answer_parallel(vector<vector<double> >* U_1, vector<vector<double> >* U_2, double tau, bool flag) {
	int rank, nodes;
	MPI_Comm_size(MPI_COMM_WORLD, &nodes);
	MPI_Comm_rank(MPI_COMM_WORLD, &rank);
	int parallel_count = (flag) ? round(this->n_x*1.0/nodes) : round(this->n_y*1.0/nodes);

	//printf("%d> size: %d; count: %d;\n", rank, nodes, parallel_count);
	...
	if (flag==0) {
		...
		int i_min = rank*parallel_count;
		int i_max = (rank < nodes-1) ? (rank+1)*parallel_count : this->n_y;
		int i = i_min;
		//printf("%d> OX: from %d to %d; ny: %d\n", rank, i_min, i_max, this->n_y);

		while(i < i_max){
			//if(rank == 1) printf("1> IN_CYCLE: i=%d; i<%d && i>=%d && i<%d\n", i, this->n_y, rank*parallel_count, (rank+1)*parallel_count);

        	int j=0, index_from, index_to, J=0;
			while (j<this->n_x) {
				//if(rank == 1) printf("IN_CYCLE_line: j=%d; j<%d\n", j, this->n_x);
				//find first not zero
				for (;j<this->n_x && this->vect_rectangle[i][j].status==0;j++);
				if (j==this->n_x) {
					//empty
		...
		int j_min = rank*parallel_count;
		int j_max = (rank < nodes-1) ? (rank+1)*parallel_count : this->n_x;
		int j = j_min;
		//printf("%d> OY: from %d to %d; nx: %d\n", rank, j_min, j_max, this->n_x);
...
void Polygon::data_exchange(vector<vector<double> > *U, bool axis){
	int nx = this->n_x;
	int ny = this->n_y;
	double R[ny][nx];
	int i,j;
	...
	//Axis = (axis) ? OY : OX;
	int count=(axis) ? round(nx*1.0/nodes) : round(ny*1.0/nodes);

	MPI_Datatype vect_x;
	MPI_Type_contiguous(nx, MPI_DOUBLE, &vect_x);
	MPI_Type_commit(&vect_x);

	if (axis==0) {
		//OX
		int sndcount=(rank<nodes-1) ? count : (ny-rank*count);
		int shift=rank*count;

		double sndbuf[ny][nx];
		for (i=0;i<sndcount;i++){
			for(j=0;j<nx;j++){
				sndbuf[i][j]=(*U)[i+shift][j];
			}
		}

		MPI_Gather(sndbuf, sndcount, vect_x, R, sndcount, vect_x, root, MPI_COMM_WORLD);
		//Improve data sending when ny or nx % nodes != 0
		...
		//OY
		MPI_Datatype vect_y;
		MPI_Type_contiguous(ny, MPI_DOUBLE, &vect_y);
		MPI_Type_commit(&vect_y);

		int sndcount=(rank<nodes-1) ? count : (nx-rank*count);
		int shift=rank*count;

		double sndbuf[nx][ny];
		for(i=0;i<sndcount;i++){
			for(j=0;j<ny;j++){
				sndbuf[i][j]=(*U)[j][i+shift];
			}
		}

		double RT[nx][ny];
		MPI_Gather(sndbuf, sndcount, vect_y, RT, sndcount, vect_y, root, MPI_COMM_WORLD);
		//Improve data sending when ny or nx % nodes != 0
	...
	MPI_Bcast(R, ny, vect_x, root, MPI_COMM_WORLD);
	for(i=0; i<ny; i++){
		for(j=0; j<nx; j++){
			(*U)[i][j]=R[i][j];
		}
